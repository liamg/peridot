	...
	aptBuiltin := module.NewFactory("apt").
		...
		WithRequiresInstallFunc(func(vars variable.Collection) bool {
			for _, pkg := range vars.Get("packages").AsList().All() {
				if err := run.Run(fmt.Sprintf("apt -qq list %s | grep -q '\\[installed\\]'", pkg.AsString()), "/", false); err != nil {
					return true
				}
			}
			return false
		...
		WithInstallFunc(func(vars variable.Collection) error {
			if err := run.Run("apt update", "/", true); err != nil {
				return fmt.Errorf("failed to sync package db: %s", err)
			}
			for _, pkg := range vars.Get("packages").AsList().All() {
				if err := run.Run(fmt.Sprintf("apt -qq list %s | grep -q '\\[installed\\]' || apt install -y %s", pkg.AsString(), pkg.AsString()), "/", true); err != nil {
					return err
				}
			}
			return nil
